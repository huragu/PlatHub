const YouTubeEngine = (() => {
  ...
  function load(el, videoId, volume, autoplay) {
    ...
    endFired = false;   // new track — reset the guard regardless of which path below runs

    // If a player already exists for this SAME host element, reuse it by
    // swapping the loaded video instead of destroying and recreating the
    // iframe. This matters because destroying the iframe destroys the
    // <video> element inside it — if the user had started a browser-native
    // Picture-in-Picture session on that element (via right-click →
    // "Picture-in-Picture"), destroying it orphans the PiP window and
    // closes it. Reusing the SAME player/iframe keeps that element stable
    // across track changes, so PiP can keep following it.
    if (player && ready && hostEl === el) {
      try {
        if (autoplay) {
          player.loadVideoById(videoId);   // loads AND starts playing
        } else {
          player.cueVideoById(videoId);    // loads without auto-playing
        }
        try { player.setVolume(Math.round(volume * 100)); } catch (_) {}
        return;
      } catch (e) {
        // If reuse fails for any reason, fall through to a full rebuild.
      }
    }
