...
- **YouTubeの広告について**：PlatHubはYouTube公式のIFrame Player APIを使っており、
  広告の有無を制御する機能はAPI自体に存在しません。広告が出るかどうかは3つの独立した要因によります。
  (1) 動画自体に広告が入っているか — チャンネル側がその動画で広告を有効にしていなければ誰が見ても出ません。
  (2) YouTube Premiumアカウントでログインしているか — Premiumはサーバー側で広告を除去する仕組み。
  [YouTube公式ヘルプ](https://support.google.com/youtube/answer/7437519)は、埋め込み動画で広告が
  出る場合の対処として「YouTubeのCookieをブロックしていないか確認する」ことを案内しており、
  Premium＋Cookie許可なら埋め込みでも広告なしになることをYouTube自身が想定していると読み取れます。
  ただし保証ではなく、公式コミュニティには「Premium契約中でも広告が出た」という報告も複数あります。
  (3) 広告ブロッカー拡張機能 — Premiumとは別の無料の代替手段ですが、PlatHubでの動作は未検証で、
  YouTube側もブロッカー検出を強化しているため安定した方法ではありません。改善しない場合はPlatHub側では
  これ以上対処できません（設定画面の「YouTube」セクションにも同じ案内と出典リンクがあります）。
